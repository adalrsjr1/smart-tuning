...
However, the decoupling has the cost of make difficulty the application management considering the large number of
distributed pieces of software. These services are mostly bundled up with several runtime abstraction layers to bring
some facility to their execution or management at runtime. Some examples of these layers are application servers (e.g.,
OpenLiberty), runtime environment (e.g., Java Virtual Machine), machine isolation (e.g., containers and virtual
machines), and cluster orchestrator (e.g., Kubernetes).

Runtime abstractions layers are general purpose and because that they have many parameters, or knobs, that the
application's engineer can adjust to better satisfy the use cases of the application. Many of these knobs are related
to data structures of infrastructure mechanisms, so their adjustment affects the application's performance. 

However, these abstraction layers are general purpose and have many parameters that the application engineer can adjust
to satisfy the use cases of the application. BesidesThey are directly related to the applications performance

and to suit up any use case of the application engineer they are
parametrized with many options (knobs)
