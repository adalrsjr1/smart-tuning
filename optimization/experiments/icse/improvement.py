...
def plot_perf(df0: pd.DataFrame, dfN: pd.DataFrame, title=''):
    ...
    def plotter2(ax: Axes, df0, dfN, metric, ylabel='', yunit=''):
    ...
    def plotter(ax: Axes, df0, dfN, metric, ylabel='', yunit=''):
        data = pd.DataFrame(df0.loc[metric])
        # data.columns = ['initial cfg']
        data['final cfg'] = pd.DataFrame(dfN.loc[metric]-1)
        d = pd.DataFrame(dfN.loc[metric]-1)
        b = d.copy()
        b[b > 0] = 0
        b = b.rename(columns={metric: 'decrease'})
        u = d.copy()
        u[u < 0] = 0
        u = u.rename(columns={metric: 'increase'})

        posbar = u.plot.bar(ax=ax, bottom=0, color='C0')
        negbar = b.plot.bar(ax=ax, bottom=0, color='C1')
        
        # py = 0
        # for p in ax.patches:
        #     if p.get_height() != 0:
        #         color = 'black'
        #         ax.annotate(f'{str(p.get_height()):.4}', (p.get_x() * 1, 0), fontsize='x-small', color=color, weight='bold')

        
        
        # 
        # ax.bar(x=np.arange(len(df0)), height=data['final cfg']-1)


        b = np.linspace(b.min().min(), 0, 1)
        u = np.linspace(0, u.max().max(), )

        diff = max(abs(b.min().min()), u.max().max())

        ygrid = [-diff, 0, diff]

        # ax.set_yticks(np.linspace(data.min().min(), data.max().max(), 4))
        # ax.set_yticks([min(-1, data.min().min()), 0, max(1, data.max().max())])
        ax.set_yticks(ygrid)
        ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right', fontsize='small')
...
def improvement(df_frameworks: list[pd.DataFrame], to_skip=[]):
    row = -1
    ...
    perf0 = {}
    for i, df in enumerate(df_frameworks):
        perf0 = perf0 | {
            f'{i}-JSP': filter(df, 'workload_jsp', row),
            f'{i}-JSF': filter(df, 'workload_jsf', row),
        }
    ...
    perfN = {}
    for i, df in enumerate(df_frameworks):
        jspn = {f'{i}-JSP': filter(df, 'workload_jsp', row)}
        jsfn = {f'{i}-JSF': filter(df, 'workload_jsf', row)}

        if f'{i}-JSP' not in to_skip:
            jspn = {f'{i}-JSP': filter(df, 'workload_jsp', row, cfg_idx=cfg_idx)}

        if f'{i}-JSF' not in to_skip:
            jsfn = {f'{i}-JSF': filter(df, 'workload_jsf', row, cfg_idx=cfg_idx)}

        perfN_to_add = jspn | jsfn
        perfN = perfN | perfN_to_add

        # perfN = perfN | {
        #     f'{i}-JSP': filter(df, 'workload_jsp', row, cfg_idx=cfg_idx),
        #     f'{i}-JSF': filter(df, 'workload_jsf', row, cfg_idx=cfg_idx),
        # }
...
def improvement_original(df_frameworks: list[pd.DataFrame], to_skip=[]):
    row = -1
    ...
    perf0 = {
        'QHD_50':        filter(df_frameworks[0], 'workload_50', row),
        'QHD_100':       filter(df_frameworks[0], 'workload_100', row),
        'QHD_200':       filter(df_frameworks[0], 'workload_200', row),
        'AcmeAir_50':    filter(df_frameworks[1], 'workload_50', 0),
        'AcmeAir_100':   filter(df_frameworks[1], 'workload_100', 0),
        'AcmeAir_200':   filter(df_frameworks[1], 'workload_200', 0),
        'Daytrader_5':   filter(df_frameworks[2], 'workload_5', row),
        'Daytrader_10':  filter(df_frameworks[2], 'workload_10', row),
        'Daytrader_50':  filter(df_frameworks[2], 'workload_50', row),
        'Daytrader JSP': filter(df_frameworks[3], 'workload_jsp', row),
        'Daytrader JSF': filter(df_frameworks[3], 'workload_jsf', row)
    }
    row = -1
    cfg_idx = -1
    perfN = {
        'QHD_50':        filter(df_frameworks[0], 'workload_50', row, cfg_idx=cfg_idx),
        'QHD_100':       filter(df_frameworks[0], 'workload_100', row, cfg_idx=cfg_idx),
        'QHD_200':       filter(df_frameworks[0], 'workload_200', row, cfg_idx=cfg_idx),
        'AcmeAir_50':    filter(df_frameworks[1], 'workload_50', row, cfg_idx=cfg_idx),
        'AcmeAir_100':   filter(df_frameworks[1], 'workload_100', row, cfg_idx=cfg_idx),
        'AcmeAir_200':   filter(df_frameworks[1], 'workload_200', row, cfg_idx=cfg_idx),
        'Daytrader_5':   filter(df_frameworks[2], 'workload_5', row, cfg_idx=cfg_idx),
        'Daytrader_10':  filter(df_frameworks[2], 'workload_10', row, cfg_idx=cfg_idx),
        'Daytrader_50':  filter(df_frameworks[2], 'workload_50', row, cfg_idx=cfg_idx),
        'Daytrader JSP': filter(df_frameworks[3], 'workload_jsp', row, cfg_idx=cfg_idx),
        'Daytrader JSF': filter(df_frameworks[3], 'workload_jsf', row, cfg_idx=cfg_idx)
    }
...
if __name__ == '__main__':
    ...
    filenames = [
        './resources/trace-daytrader-2021-09-22T02 42 28.json',                 # jsp-jsf original (no-contention)
        './resources/trace-daytrader-jspjsf-2022-08-13T15_51_49.json',          # jsp-jsf default
        './resources/trace-daytrader-jspjsf-random-2022-08-15T13_43_41.json',   # jsp-jsf random
        './resources/trace-daytrader-jspjsfd-2022-08-14T11_50_52.json',         # jsp-jsf no dependency {"training.metric.throughput": {$lt: 10}}
        './resources/trace-daytrader-jspjsfv-2022-08-14T11_51_00.json',         # jsp-jsf no vertical scaling
        './resources/trace-daytrader-jspjsfv-2022-08-17T12_24_16.json',         # jsp-jsf no memory
        './resources/trace-daytrader-jspjsf-2022-08-20T16_05_52.json',          # jsp-jsf no thread
        './resources/trace-daytrader-jspjsfv-2022-08-19T00_12_23.json',         # jsp-jsf no memory and no thread
        './resources/trace-daytrader-jspjsfw-2022-08-13T15_52_05.json',         # jsp-jsf weight 10 to resp time
        './resources/trace-daytrader-jspjsfw-2022-08-15T13_39_16.json',         # jsp-jsf weight 1000 to resp time
    ]

    filenames = [
        './resources/trace-quarkus-2021-09-14T19 46 43.json',
        './resources/trace-acmeair-2021-09-14T19 46 28.json',
        './resources/trace-daytrader-2022-08-10T13_30_11.json',
        './resources/trace-daytrader-2021-09-22T02 42 28.json'
    ]

    # filename = './resources/trace-daytrader-jspjsf-2022-08-13T15_51_49.json'

    # name = 'trace-daytrader-2021-09-22T02 42 28'

    # seip
    #name = 'trace-daytrader-jspjsf-2022-08-13T15_51_49'


    dfs = [load_file_workload(filename, iteration_lenght_minutes=10) for filename in filenames]
    ...
    # improvement(dfs, to_skip=['4-JSF', '7-JSP', '7-JSF', '9-JSP', '9-JSF'])
    improvement_original(dfs)
