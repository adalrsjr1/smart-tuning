...
\item We assume that the application can be modeled as a histogram of
  application's urls reached at runtime. Besides, we consider that this behavior
    ...
    resource consumption and application performance.  \arsj{this doesn't sound
    quite right -- we're assuming that the workload can be modeled that way, but
    not that the application itself can be}

  \item Two equal \todo{workflows -- to define workflows somewhere before this
    sentence}, i.e., both have same shape and intensity of for every bucket, may
